...
def calculate_demographic_info(df, unique_households_df):
    ...
    return {
        ...
        **{f'Adults_with_a_{condition_categories[condition]}': df[(df['chronic_condition'].str.contains("Substance Use Disorder (Alcohol, Drugs, or Both)", na=False, regex=False)) & (df['age_group'].isin(['adult', 'youth']))].shape[0] for condition in condition_categories},
        **{f'childs_with_a_{condition_categories[condition]}': df[(df['chronic_condition'].str.contains("Substance Use Disorder (Alcohol, Drugs, or Both)", na=False, regex=False)) & (df['age_group'].isin(['child', 'unknown']))].shape[0] for condition in condition_categories},
